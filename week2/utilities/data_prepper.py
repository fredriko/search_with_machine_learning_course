...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        try:
            ...
            print(f"Got ERROR while executing log query search: {error}")
        ...
        if response and response.get("hits") and response.get("hits").get("hits") and len(response["hits"]["hits"]) > 0:
            ...
            for hit in hits:
                ...
                feature_results = {"doc_id": [], "query_id": [], "sku": [], "salePrice": [], "name_match": []}
                for doc_id in query_doc_ids:
                    _features = hit["fields"]["_ltrlog"][0]["log_entry"]
                    feature_results["doc_id"].append(doc_id)  # capture the doc id so we can join later
                    feature_results["query_id"].append(_query_id)
                    feature_results["sku"].append(doc_id)  # ^^^
                    for _feature in _features:
                        _name = _feature["name"]
                        _value = _feature.get("value", -1)
                        _feature_values = feature_results.get(_name)
                        if not _feature_values:
                            _feature_values = []
                        _feature_values.append(_value)
                        feature_results[_name] = _feature_values
                frame = pd.DataFrame(feature_results)
                return frame.astype({'doc_id': 'int64', 'query_id': 'int64', 'sku': 'int64'})
        else:
            no_results[key] = query_doc_ids
        # FO: suitable return value?
        return None
        # IMPLEMENT_END
